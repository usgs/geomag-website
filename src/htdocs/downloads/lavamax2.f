...
c     performs maximum likelihood fit of 3D bi-gaussian distribution
...
c     formula 15
      ...
      rrr = cos(rmi)*cos(rmd)*cos(ri)*cos(rd) +
     +  cos(rmi)*sin(rmd)*cos(ri)*sin(rd) + sin(rmi)*sin(ri)

      dumb = rf**2*cos(ri)/((2*pi)**(1.5)*rms**3)*
     +  exp(-rf**2/(2*rms**2))*exp(-rmf**2/(2*rms**2))

      distfid = 0.5*dumb*
     +  (exp(rf*rmf*rrr/rms**2) + exp(-rf*rmf*rrr/rms**2))
...
c     formula C2
      ...
      distfi = bessi0(dumbc)*cosh(dumbs)
...
c     formula A5
     ...
     +  cos(rmi)*sin(rmd)*cos(ri)*sin(rd) + sin(rmi)*sin(ri)
      ...
      distid  = cos(ri)*(rmfs**2*dumb**2 + 1)*
     +  exp((dumb**2-1)*rmfs**2/2.)/(4*pi)
